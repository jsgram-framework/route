...
class Node
{
	...
	public find(path: string): [any, {}]
	{
		...
		const originalPathLength: number = path.length;
		...
		while (true) {
			...
			if (pathLen === 0 || path === prefix) {
				// found the route
				const handle = currentNode.handle.asset;
				...
				if (handle !== null && handle !== undefined) {
					...
					if (currentNode.handle.paramNames.length > 0) {
						for (let j = 0; j < currentNode.handle.paramNames.length; j++) {
							const key = currentNode.handle.paramNames[j];
			...
			if (node === null) {
				...
				const goBack: string = previousPath.charCodeAt(0) === 47 ? previousPath : "/" + previousPath;
				...
				if (originalPath.indexOf(goBack) === -1) {
					...
					const pathDiff: string = originalPath.slice(0, originalPathLength - pathLen);
			...
			if (type === REGEX) {
				...
				const check = originalPath.slice(idxInOriginalPath, idxInOriginalPath + i);
	...
	public add(route: Route, standAlone = false): void
	{
		let path = route.path;
		const handle = route.routeId;
		const routeRegex = route.regexParts;
		const params = [];
		let j = 0;
		...
		for (let i = 0, len = path.length; i < len; ++i) {
			...
			if (path.charCodeAt(i) === 58) {
				...
				if (standAlone) {
					...
					const regexParam = isRegex ? parameter.slice(parameter.indexOf("("), i) : null;
